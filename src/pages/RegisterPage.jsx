...
function RegisterPage() {
  const handleSubmit = (event) => {
    event.preventDefault(); // Mencegah reload halaman standar saat form disubmit
    const formData = new FormData(event.target);
    const data = Object.fromEntries(formData.entries());
    // console.log('Registration Data:', data); // Menampilkan data form di console
    // alert('Account created successfully (simulation)!'); // Notifikasi simulasi
  };
      ...
      <div className="md:w-1/2 w-full max-w-md"> 
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="text-sm text-gray-600 pt-2 text-left"> {/* Memastikan teks rata kiri */}
            ...
            <Link to="/login" className="font-medium text-[#2859A6] hover:text-[#2859A6] hover:underline">
              Sign in
            </Link>
          </div>
